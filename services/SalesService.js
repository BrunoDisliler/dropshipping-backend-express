...
const { generalValidation } = require('../validations/validation');
...
const create = async (sales) => {
  const invalidSale = validation.saleValidation(sales);
  if (invalidSale) return invalidSale;

  const invalidProduct = await validation.productsValidation(sales);
  if (!invalidProduct) {
...
const update = async ({ id }, sales) => {
  ...
  await Promise.all(
    sales.map((sale) => salesModel.update(id, sale.productId, sale.quantity)),
  );
